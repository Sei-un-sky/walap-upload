...
`0.2.1` 版本只做一件事：把完整世界目录打包成 zip，然后上传到远端存储。插件不提供自动回档，也不会自动覆盖世界目录。需要回档时，管理员手动下载备份、停服、替换存档。
...
- 按间隔自动备份
- 把完整世界目录打成一个 zip
- 创建快照前后执行存档命令：`save-off`、`save-all flush`、复制快照、`save-on`
- `save-on` 之后才开始压缩和上传，所以这两个阶段游戏都可以继续正常保存
- 上传后端独立模块化
